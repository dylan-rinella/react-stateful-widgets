...
export default function Programmers() {
  ...
  const [featuredProgrammerId, setFeaturedProgrammerId] = useState(null)
  ...
  const getNameOfFeatured = () => {
    ...
    setFeaturedProgrammerId(programmers.name)
      ...
      <div className='programmers'>
        {
          ...
          programmers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { setFeaturedProgrammerId(dev.id) }}>Feature</button>
            </div>
          )
        }
      </div>
